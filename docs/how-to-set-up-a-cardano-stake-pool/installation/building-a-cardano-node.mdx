...
```bash
# Get latest tag with wget + grep (no curl)
TAG=$(wget -qO- https://api.github.com/repos/Love2Stake/love2automate-ada/releases/latest | grep -oP '"tag_name": "\K[^"]+')

# Define the asset name
ASSET_NAME="love2automate-ada-linux-x64.tar.gz"

# Download asset with wget using tag and asset name
...
sudo mv love2automate-ada /usr/local/bin
```
<br />
...
### 3. Setting up Love2Automate's Dependencies

The following command with install all of the required dependencies for the functions of Love2Automate:
